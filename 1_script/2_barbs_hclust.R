...
##### tree cut
current_treecut <- data.frame(ID_barb = ward_hclust$labels,
...
ggsave(b,
       filename = file.path(output_dir, "barbs_PCA_pc1pc2.svg"),
       width = barbs_PCA_pc1pc2_width,
       height = barbs_PCA_pc1pc2_height,
       units = "in")
ggsave(b,
       filename = file.path(output_dir, "barbs_PCA_pc1pc2.png"),
       width = barbs_PCA_pc1pc2_width,
       height = barbs_PCA_pc1pc2_height,
       units = "in")
...
closed_open_outlines_w_cluster <- Momocs::Out(x = open_outlines_w_cluster$coo,
                                              fac = open_outlines_w_cluster$fac)
...
chronozones_TP_df$ID <- as.factor(chronozones_TP_df$ID)
chronozones_TP_df$ID_barb <- as.factor(chronozones_TP_df$ID_barb)
chronozones_TP_df$Location <- as.factor(chronozones_TP_df$Location)
chronozones_TP_df$Period <- as.factor(open_outlines_w_cluster$fac$Period)
chronozones_TP_df$Typology <- as.factor(chronozones_TP_df$Typology)
rownames(chronozones_TP_df) <- current_treecut$ID_barb
...
ggsave(tree_w_clusterlabels,
       filename = file.path(output_dir, "barbs_tree_w_clusterlabels.svg"),
       width = tree_w_clusterlabels_width,
       height = tree_w_clusterlabels_height,
       units = "in")
ggsave(tree_w_clusterlabels,
       filename = file.path(output_dir, "barbs_tree_w_clusterlabels.png"),
       width = tree_w_clusterlabels_width,
       height = tree_w_clusterlabels_height,
       units = "in")
