...
##### tree cut
current_treecut <- data.frame(ID = row.names(as.data.frame(cutree(ward_hclust,
                                                                  k = n_clusters))),
                              cluster = as.factor(as.data.frame(cutree(ward_hclust,
                                                                       k = n_clusters))[[1]]))

current_treecut <- dplyr::left_join(current_treecut, point_info, by = "ID")

# open_outlines_w_cluster <- open_outlines
# open_outlines_w_cluster$fac$Cluster <- current_treecut$cluster
# 
# open_outlines_w_cluster_PCA <- open_outlines_PCA
# open_outlines_w_cluster_PCA$fac$Cluster <- current_treecut$cluster
...
for (cluster_index in 1:n_clusters){
  mean_shapes_cluster[[cluster_index]] <- Momocs::MSHAPES(Momocs::coo_interpolate(Momocs::slice(open_outlines_w_cluster, cluster == cluster_index),
                                                                                  n = min_no_of_coordinates[[cluster_index]])$coo)
}

open_outlines_w_cluster_mean_shapes_cluster_out <- Momocs::Out(mean_shapes_cluster,
                                                               fac = data.frame(cluster = paste0("cluster_", c(1:n_clusters))))
...
for (i in 1:n_clusters){
  ...
  Momocs::panel(current_panel,
                main = NULL,
                col = rep(color_palette[i], length(current_panel)),
                names = T)
...
chronozones_TP_df <- open_outlines_w_cluster$fac
...
rownames(chronozones_TP_df) <- current_treecut$ID

# subset(open_outlines_w_cluster_PCA_df, Cluster == 3) # check the IDs for each cluster to find the right clades
...
tree_w_clusterlabels <- ggtree(ward_hclust) %<+% chronozones_TP_df + 
  ...
  geom_cladelabel(node=33, label="Cluster 1", align=T, geom='text', offset = 2.5, color=c("#E69F00"), barsize = 2, fontsize = 6) + 
  geom_cladelabel(node=26, label="Cluster 2", align=T, geom='text', offset = 2.5, color=c("#56B4E9"), barsize = 2, fontsize = 6) + 
  geom_cladelabel(node=30, label="Cluster 3", align=T, geom='text', offset = 2.5, color=c("#009E73"), barsize = 2, fontsize = 6) +
  geom_cladelabel(node=8, label="Cluster 4", align=T, geom='text', offset = 2.5, color=c("#FF6347"), barsize = 2, fontsize = 6) +
  geom_cladelabel(node=28, label="Cluster 5", align=T, geom='text', offset = 2.5, color=c("#0072B2"), barsize = 2, fontsize = 6) + 
  geom_cladelabel(node=32, label="Cluster 6", align=T, geom='text', offset = 2.5, color=c("#D55E00"), barsize = 2, fontsize = 6) +
  geom_cladelabel(node=15, label="Cluster 7", align=T, geom='text', offset = 2.5, color=c("#CC79A7"), barsize = 2, fontsize = 6) +
  xlim_tree(46) +
